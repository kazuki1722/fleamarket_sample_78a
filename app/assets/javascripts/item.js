$(function(){
  ...
  fileIndex.splice(0, lastIndex);
    ...
    if (img = $(`img[data-index="${targetIndex}"]`)[0]) {
      ...
      $("#image-label").prop("for", `item_item_images_attributes_${fileIndex[0]}_image`)
      fileIndex.shift();
      // 末尾の数に1足した数を追加する
...
// ================== 削除ボタン押した時 ===========================================
  $('#image-box').on('click', '.js-remove', function() {
    ...
    const fileinput =$(`#item_item_images_attributes_${targetIndex}_image`);
    $(this).parent().remove();
    ...
    //画像をクリックするとその画像と新しい画像を差替える。 削除をクリックすると(`#item_item_images_attributes_${index}_image`)のindexが削除ボタンの頭上にある画像を識別し、削除する。
